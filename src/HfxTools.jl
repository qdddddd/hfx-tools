...
using .Threads, Logging, CSV, ProgressMeter, Printf, Glob, DataFrames, DataStructures, Dates, GZip, JSON, StatsBase, JlUtils
...
function process(args::ProcessArgs, file_lock::LockedDict{String,ReentrantLock}, out_store::Dict{String,Deque{DataFrame}})
    ...
    if !isnothing(args.start_time)
        filter!(:Timestamp => (x -> Time(unix2datetime_adj(x)) >= args.start_time), df_raw)
    end
...
function main()
    ...
    for i in eachindex(ARGS)
        ...
        elseif arg == "--func"
            func = getfield(HfxTools, Symbol(ARGS[i+1]))
        elseif arg == "--cor-func"
    ...
    if func == corr_label
        ...
        @showprogress desc = "Loading downsampled timestamps" showspeed = true barlen = 60 for file in readdir(selected_root)
            ...
            if !isnothing(dict_existing_end)
                filter!(:Timestamp => >(dict_existing_end[file[1:9]]), df)
            end
            lock(lk) do
